import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import os
import warnings

# Suppress FutureWarning from Plotly
warnings.filterwarnings('ignore', category=FutureWarning)

# Set page config
st.set_page_config(page_title="Competitor Hiring Intelligence", layout="wide")

# Title
st.title("🎯 Competitor Hiring Intelligence Dashboard")

# Load data function with better error handling
@st.cache_data
def load_data():
    try:
        # Try multiple possible paths for Streamlit Cloud
        possible_paths = [
            "data/output/clustered_jobs.csv",
            "src/data/output/clustered_jobs.csv", 
            "clustered_jobs.csv",
            r"D:\compititor_hiring_analysis\data\output\clustered_jobs.csv"
        ]
        
        keyword_possible_paths = [
            "data/output/keyword_summary.csv",
            "src/data/output/keyword_summary.csv",
            "keyword_summary.csv", 
            r"D:\compititor_hiring_analysis\data\output\keyword_summary.csv"
        ]
        
        df = None
        for path in possible_paths:
            if os.path.exists(path):
                df = pd.read_csv(path)
                break
        
        keyword_df = None
        for path in keyword_possible_paths:
            if os.path.exists(path):
                keyword_df = pd.read_csv(path)
                break
                
        return df, keyword_df
    except Exception as e:
        st.error(f"Error loading data: {str(e)}")
        return None, None

# Create sample data if files don't exist
def create_sample_data():
    st.warning("Data files not found. Using sample data for demonstration.")
    
    # Sample data structure
    sample_data = {
        'competitor': ['Deel', 'CXC Global', 'Remote', 'Multiplier'] * 10,
        'department': ['Sales', 'Tech', 'Marketing', 'Payroll'] * 10,
        'region': ['EU', 'APAC', 'LATAM', 'EMEA'] * 10,
        'count': [25, 15, 8, 12] * 10,
        'cluster_name': ['Platform Innovators', 'Digital Transformation', 'Remote Solutions', 'EOR Specialists'] * 10
    }
    
    df = pd.DataFrame(sample_data)
    
    # Sample keyword data
    keyword_data = {
        'competitor': ['Deel', 'CXC Global', 'Remote', 'Multiplier'],
        'compliance': [0.8, 0.6, 0.4, 0.7],
        'payroll': [0.9, 0.5, 0.3, 0.6],
        'integration': [0.6, 0.8, 0.5, 0.4],
        'management': [0.7, 0.7, 0.6, 0.5],
        'operations': [0.5, 0.6, 0.7, 0.8]
    }
    
    keyword_df = pd.DataFrame(keyword_data)
    
    return df, keyword_df

# Try to load data, use sample if not available
df, keyword_df = load_data()

if df is None:
    df, keyword_df = create_sample_data()

if df is not None:
    # Executive summary
    st.header("📊 Strategic Insights")
    st.markdown("""
    - **Deel**: Aggressively expanding with **227 roles**, focusing on **sales/marketing (72)** and **payroll (48)** in **EU, APAC, LATAM**.
    - **CXC Global**: **12 tech roles** in **APAC**, building a **digital transformation hub** (e.g., ServiceNow, MuleSoft).
    - **Remote**: **4 roles** in **EMEA/Global**, pushing **remote-first client acquisition**.
    - **Multiplier**: **4 roles** in **APAC/LATAM**, developing **embedded EOR solutions**.
    - **Action**: Prioritize enterprise pitches against Deel; highlight compliance in LATAM against Multiplier.
    """)

    # Sidebar filters
    st.sidebar.header("Filters")
    competitor = st.sidebar.multiselect("Select Competitor", options=df['competitor'].unique(), default=df['competitor'].unique())
    department = st.sidebar.multiselect("Select Department", options=df['department'].unique(), default=df['department'].unique())
    region = st.sidebar.multiselect("Select Region", options=df['region'].unique(), default=df['region'].unique())
    
    # Filter data
    filtered_df = df[df['competitor'].isin(competitor) & df['department'].isin(department) & df['region'].isin(region)]
    
    # Role counts by competitor
    st.header("📈 Competitor Hiring Volume")
    role_counts = filtered_df.groupby('competitor')['count'].sum().reset_index()
    fig1 = px.bar(role_counts, x='count', y='competitor', title="🚀 Top Hiring Competitors", color='competitor')
    st.plotly_chart(fig1, use_container_width=True)
    st.markdown("**What This Means**: Deel's 227 roles signal intense enterprise competition; CXC's APAC tech focus offers niche opportunities.")

    # Department distribution
    st.header("🏢 Department Distribution")
    dept_counts = filtered_df.groupby('department')['count'].sum().reset_index()
    fig2 = px.bar(dept_counts, x='count', y='department', title="🛠 Department Hiring Trends", color='department')
    st.plotly_chart(fig2, use_container_width=True)
    st.markdown("**What This Means**: High Sales/Payroll hiring (Deel) suggests market expansion; emphasize compliance in pitches.")

    # Heatmap
    st.header("🗺️ Hiring by Region and Department")
    try:
        pivot = filtered_df.pivot_table(values='count', index='competitor', columns=['region', 'department'], aggfunc='sum', fill_value=0)
        fig3 = go.Figure(data=go.Heatmap(z=pivot.values, x=[f"{r}-{d}" for r, d in pivot.columns], y=pivot.index, colorscale='Blues'))
        fig3.update_layout(title="🌍 Hiring Volume by Competitor, Region, and Department")
        st.plotly_chart(fig3, use_container_width=True)
        st.markdown("**What This Means**: Deel's Global Payroll/Sales focus requires enterprise-grade solutions; CXC's APAC Tech hiring demands integration expertise.")
    except Exception as e:
        st.error(f"Error creating heatmap: {str(e)}")

    # Top keywords
    st.header("🔍 Top Keywords by Competitor")
    if keyword_df is not None:
        try:
            fig4 = px.bar(keyword_df.melt(id_vars='competitor', var_name='keyword', value_name='tfidf_score'), 
                          x='competitor', y='tfidf_score', color='keyword', barmode='group')
            fig4.update_layout(title='💡 Top Keywords by Competitor', xaxis_title='Competitor', yaxis_title='TF-IDF Score')
            st.plotly_chart(fig4, use_container_width=True)
        except Exception as e:
            st.error(f"Error creating keyword chart: {str(e)}")
    else:
        st.write("Top Keywords: compliance, payroll, integration, management, operations")
        st.warning("Keyword data not found. Run the notebook to generate 'keyword_summary.csv'.")
    st.markdown("**What This Means**: Deel's 'compliance' focus suggests EOR priority; Multiplier's 'EOR' emphasis targets LATAM.")

    # Clusters
    st.header("🎯 Hiring Theme Clusters")
    if 'cluster_name' in filtered_df.columns:
        cluster_counts = filtered_df.groupby(['competitor', 'cluster_name'])['count'].sum().reset_index()
        fig5 = px.bar(cluster_counts, x='competitor', y='count', color='cluster_name', title="🤖 Competitor Hiring Themes")
        st.plotly_chart(fig5, use_container_width=True)
        st.markdown("**What This Means**: 'Platform Innovators' (Deel, Multiplier) focus on tech/EOR; compete with innovative features.")
    else:
        st.warning("Cluster data not available in the dataset.")
        
else:
    st.error("Unable to load data. Please check if the data files exist in the correct location.")
